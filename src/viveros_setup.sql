...
create table if not exists telefonos_viveros (
  ...
  telefono VARCHAR(15) primary key check (telefono ~ '^[+]?([0-9]{1,3})?[0-9]{6,12}$')
);
...
delete from viveros where id_vivero = 1;
select * from viveros;
select * from telefonos_viveros;
select * from zonas;
select * from stock;
...
delete from cliente_tajinaste_plus where dni_cliente = '12345678A';
select * from cliente_tajinaste_plus;
select * from pedidos;
...
delete from empleados where dni_empleado = '33333333C';
select * from empleados;
select * from pedidos;
select * from historial_trabajo;

-- Eliminación de una tarea
delete from tareas where nombre_tarea = 'Trasplante';
select * from tareas;
select * from historial_trabajo;